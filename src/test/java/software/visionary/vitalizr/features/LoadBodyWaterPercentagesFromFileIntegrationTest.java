...
import software.visionary.vitalizr.bodyWater.BioelectricalImpedanceConverter;
import software.visionary.vitalizr.bodyWater.BioelectricalImpedanceSerializationProxy;
...
class LoadBodyWaterPercentagesFromFileIntegrationTest {
    @Test
    void canLoadBodyWaterPercentagesFromFile() throws IOException {
        // Given: A file containing some vitals for a person
        final Person mom = Fixtures.createRandomPerson();
        final BodyWaterPercentage toStore2 = new BioelectricalImpedance(Instant.now().plus(-2, ChronoUnit.DAYS), 33.1, mom);
