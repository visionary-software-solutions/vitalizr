...
import software.visionary.vitalizr.bodyTemperature.ImperialTemperatureSerializationProxy;
...
class LoadBodyTemperaturesFromFileIntegrationTest {
    @Test
    void canLoadBodyTemperaturesFromFile() throws IOException {
        // Given: A file containing some vitals for a person
        final Person mom = Fixtures.createRandomPerson();
        final BodyTemperature toStore2 = new ImperialTemperature(mom, 33.1, Instant.now().plus(-2, ChronoUnit.DAYS));
